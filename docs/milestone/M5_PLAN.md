...
## 目的
- リポジトリ直下の `Tasks.md` を M5 作業管理用にし、完了済み M4 の詳細チェックリストは `docs/tasks/M4_PLAN.md` へ集約する。
...
- 対象:
  ...
  - 作業管理: ルート `Tasks.md`、`docs/tasks/M4_PLAN.md`
- 非対象:
...
- 変更時は `Tasks.md` の M5-003 を更新し、必要なら `architecture.md` の用語と揃える。

## タスク分解（ルート `Tasks.md` と対応）

### M5-001: README の現行化
- [x] 上記方針に沿って `README.md` を編集する。

### M5-002: アーキテクチャ文書
- [x] `docs/architecture.md` を作成し、Mermaid 図を 2 本以上含める。
...
## 受け入れ基準
- M4 の詳細チェックリストが `docs/tasks/M4_PLAN.md` にあり、ルート `Tasks.md` は M5 に特化している。
