...
- [x] `build_diff_highlight_overlay_rgba` を **M6 初期の画素差分＋開閉・膨張**のみに戻す（インク XOR／same-cell 補助パラメータは削除）。
- [x] `main_tab` の差分オーバーレイは **二色化／指定色濃淡で分岐しない**（処理済みフル解像度ラスタのみ使用）。パレット色は平均して単一 `highlight_rgba` に反映。

## M8-D: プレビュー進捗と変身後のページ位置

- [x] **ページ遷移**（前へ／次へ／ページ番号入力）でプレビュー再描画中にプログレスバー（`U182`）。
- [x] **変身ボタン**で変換完了後、プレビュー再描画中にプログレスバー（`U183`）。PDF→PNG 変換自体は既存の `ProgressWindow` を継続利用。
- [x] **二値化／指定色濃淡**プルダウン変更時にプログレスバー（`U184`）。
- [x] **差分を強調表示**チェック切替時にプログレスバー（`U185`）。
- [x] 変身（DPI 反映）後の `current_page_index` を、変身**前**のインデックスに**クランプして維持**（`page_count` 減少時は末尾に合わせる）。
